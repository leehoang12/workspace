...
void displayBuffer (char buffer[], int sizeBuffer){
	int result1, resultX, resultY;
	int i;
	int keyPress;
	
	switch (buffer[1]) {
	case '/':
		result1 = 2 | isLongpress;		// Right Click  -- finger 2 press
		resultX = 0;
		resultY = 0;
		isMouse = 1;
		break;
	case '*':			// Left Click  -- double tab
		result1 = 1;
		resultX = 0;
		resultY = 0;
		isMouse = 1;
		break;
	case '+':			// Right Click  -- Long press
		isLongpress = 1;
		result1 = 1;
		resultX = 0;
		resultY = 0;
	...
	case '@':
		isMouse = 0;
		PORTDbits.RD1 = 1 - PORTDbits.RD1;

		i=2;
		keyPress = 0;
		while(i<sizeBuffer-1){
			if (buffer[i]<'0' || buffer[i]>'9'){
				keyPress = 0;
				break;
			} else {
				keyPress = keyPress*10 + (buffer[i] - '0');
				i++;
			}
		}
		...
		hid_report_in2[0] = 0;
       	hid_report_in2[1] = 0;
		hid_report_in2[2] = keyPress;  // tam thoi
        hid_report_in2[3] = 0;
        hid_report_in2[4] = 0;
        hid_report_in2[5] = 0;
        hid_report_in2[6] = 0;
	...
	if (isMouse){
		hid_report_in1[0] = result1;
		hid_report_in1[1] = resultX;
    	hid_report_in1[2] = resultY;
		HIDTxPacket(HID_EP1, (BYTE*)hid_report_in1, 0x03);
	}
}
